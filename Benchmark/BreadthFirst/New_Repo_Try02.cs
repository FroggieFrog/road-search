...
{
    public static class New_Repo_Try02
    {
            ...
            AnnoObject[][] gridDictionary = null)
        {
            ...
            foreach (var startObject in startObjects)
            {
                ...
                var startPositionX = (int)startPosition.X;
                var startPositionY = (int)startPosition.Y;
                var startRemainingDistance = rangeGetter(startObject);

                for (var i = 0; i < startSize.Width; i++)
                {
                    searchedCells.Enqueue((startRemainingDistance, i + startPositionX, startPositionY - 1));
                    searchedCells.Enqueue((startRemainingDistance, i + startPositionX, (int)(startPositionY + startSize.Height)));

                    visitedCells[i + startPositionX][startPositionY - 1] = true;
                    visitedCells[i + startPositionX][(int)(startPositionY + startSize.Height)] = true;
                }

                for (var i = 0; i < startSize.Height; i++)
                {
                    searchedCells.Enqueue((startRemainingDistance, startPositionX - 1, i + startPositionY));
                    searchedCells.Enqueue((startRemainingDistance, (int)(startPositionX + startSize.Width), i + startPositionY));

                    visitedCells[startPositionX - 1][i + startPositionY] = true;
                    visitedCells[(int)(startPositionX + startSize.Width)][i + startPositionY] = true;
                }
                ...
                for (var i = 0; i < startSize.Width; i++)
                {
                    ...
                    {
                        visitedCells[startPositionX + i][startPositionY + j] = true;
                    }
                }
            }
